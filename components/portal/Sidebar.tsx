...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="space-y-2">
      <div className="px-3 pt-4 text-[11px] font-semibold tracking-wide text-gray-400 uppercase">{title}</div>
      {children}
    </div>
  );
}
function NavList({ pathname, items, onItemClick }: { pathname: string; items: Item[]; onItemClick?: () => void }) {
  return (
          ...
          <li key={it.href}>
            <Link
              ...
              className={cx(
                ...
                active ? "bg-sky-50 text-sky-900 border-l-2 border-sky-600" : "text-gray-700 hover:bg-gray-50 border-l-2 border-transparent"
              )}
            >
              {Icon ? <Icon className={cx(active ? "text-sky-700" : "text-gray-500 group-hover:text-gray-700")} /> : null}
              <span className="flex-1 truncate">{it.label}</span>
              {it.badge ? <span className="ml-2 rounded-full bg-sky-100 text-sky-800 text-[10px] px-2 py-0.5">{it.badge}</span> : null}
