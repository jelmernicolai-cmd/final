...
export default function LOEPlannerLite() {
  ...
  const resetBoth = () => {
    setScenarios([
      { id: "A", name: "Scenario A (Base)", color: COLORS.A, inputs: { ...DEFAULTS } },
      { id: "B", name: "Scenario B", color: COLORS.B, inputs: { ...DEFAULTS } },
    ]);
    setActiveId("A");
  };
  ...
  const exportCSV = () => {
    ...
    [
      { name: sA.name, points: simA.points },
      { name: sB.name, points: simB.points },
    ].forEach(({ name, points }) => {
      points.forEach(p => {
        rows.push([
          name,
              ...
              {[{ sc: sA, sim: simA }, { sc: sB, sim: simB }].map(({ sc, sim }) => {
                ...
                return (
                  <tr key={sc.id} className="border-b">
                    ...
                    <td className="py-2 pr-4">{pctS(sim.kpis.avgShareY1, 1)}</td>
                    <td className="py-2 pr-4">{pctS(sim.kpis.endShare, 1)}</td>
                    <td className="py-2 pr-4">{eur(sim.kpis.endNet, 0)}</td>
                    <td className="py-2 pr-4">{pctS(sim.kpis.volLossY1, 1)}</td>
                    <td className="py-2 pr-4">{pctS(sim.kpis.gtnLeakY1, 1)}</td>
