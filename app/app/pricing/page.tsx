...
const pct = (p: number) =>
  `${Number.isFinite(p) ? Math.round(p * 100) : 0}%`;
...
export default function PricingAdminPage() {
  ...
  const kpi = useMemo(() => {
    ...
    const caseFilled = rows.filter((r) => {
      ...
      const q = num(c, NaN);
      // casePack mag ook string zijn (bijv. "6 x 10"), dan tellen we invulling ≠ lege string
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 space-y-6">
      ...
      <section className="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(200px,1fr))]">
        ...
        <KpiCard
          title="Case/Doos ingevuld"
          value={pct(kpi.caseCov)}
          hint="Deel met ingevulde doosverpakking"
        />
        <KpiCard
          title="Openstaande validaties"
          value={String(kpi.invalid)}
          hint={kpi.invalid ? "Los de rode velden op in AIP" : "OK"}
          tone={kpi.invalid ? "warn" : "ok"}
        />
      </section>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Card
          ...
          footer={
            <div className="text-xs text-gray-500">
              Snapshot: {kpi.withAip}/{kpi.n} met geldige AIP
            </div>
          }
        />
        ...
        <Card
          ...
          ctaLabel="Openen"
          // ← hier de geüpdatete link naar je Wgp-builder UI
          href="/app/wgp"
          tone="info"
  ...
  return (
    <div className={`rounded-2xl border p-4 ${info ? "border-sky-200 bg-white" : "bg-white"}`}>
      ...
      <div className="mt-3 flex items-center justify-between">
        <Link href={href} className={`inline-flex items-center rounded-lg ${info ? "bg-sky-600 text-white hover:bg-sky-700" : "border hover:bg-gray-50"} px-3 py-2 text-sm`}>
          {ctaLabel}
        </Link>
        {footer}
      </div>
