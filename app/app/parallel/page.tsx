...
type Point = {
  ...
  netSalesOriginator: number; // = grossOriginator − discountSpend
    ...
    <main className="max-w-screen-xl mx-auto px-3 sm:px-4 lg:px-6 py-4 space-y-6">
      ...
      <header className="rounded-2xl border bg-white p-4 sm:p-5">
        ...
        <p className="text-sm text-gray-700 mt-1">
          We rekenen uitsluitend met <b>Net Sales = Gross − Discounts</b>. Minder PI stroomt <b>1-op-1</b> terug naar originator.
          Stel één <b>korting %</b> en optioneel een <b>handmatige PI-share</b> in; vergelijk A vs B en vind je <b>break-even korting</b>.
        </p>
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          <div className="rounded-xl border p-3">
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
              <span title="Eind-gap koperprijs vs parallel">
                Gap: <b>{eur(simA.kpis.endGap, 0)}</b>
              </span>
              <span title="Eind-PI share">
                PI: <b>{pctS(simA.kpis.endPIshare)}</b>
              </span>
              <span title="Netto koperprijs (einde)">
                Net prijs: <b>{eur(simA.kpis.endNetPrice, 0)}</b>
              </span>
            </div>
          </div>
          <div className="rounded-xl border p-3">
          ...
          <div className="rounded-xl border p-4">
            ...
            <div className="grid gap-3 [grid-template-columns:repeat(auto-fit,minmax(180px,1fr))]">
              <Kpi title="Gap (einde)" value={eur(simA.kpis.endGap, 0)}
                   ...
                   titleTooltip="Koperprijs t.o.v. parallel referentie" />
      ...
      <section className="rounded-2xl border bg-white p-4">
        ...
        <p className="text-xs text-gray-600 mt-2">
          We hanteren <b>sOriginator = 1 − sPI</b>: minder PI stroomt 1-op-1 terug naar originator. Netto = Gross − Discount.
        </p>
      </section>

      {/* Grafieken – responsief in twee kolommen op groot scherm, stapelen op mobiel */}
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border bg-white p-4">
          ...
          <LineChart
            name="PI share A"
            color="#6366f1"
            values={simA.points.map(p => p.sPI * 100)}
            yFmt={(v) => `${v.toFixed(0)}%`}
          />
          <div className="mt-2" />
          <LineChart
            name="PI share B"
            color="#f59e0b"
            values={simB.points.map(p => p.sPI * 100)}
            yFmt={(v) => `${v.toFixed(0)}%`}
          />
        </div>
      </section>
