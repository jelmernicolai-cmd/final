...
export default function Page() {
  ...
  async function runTargetedScan() {
    ...
    if (!f) { setErr("Kies eerst een Staatscourant PDF."); return; }
    if (!aip.length) { setErr("Upload eerst de AIP-master."); return; }
    setErr(null); setBusy(true); setProgress({done:0,total:0});
    ...
    <div className="mx-auto max-w-6xl px-3 sm:px-4 lg:px-6 py-6 space-y-6">
      <header className="rounded-2xl border bg-white p-4 sm:p-5">
        ...
        <p className="text-sm text-gray-700 mt-1">
          Upload je <b>AIP-master</b> en de <b>Staatscourant-PDF</b>. We scannen pagina’s gericht op jouw REGNR’s
          en nemen de prijs van die pagina over. Geen server, werkt ook met héle grote PDF’s.
        </p>
      </header>
      ...
      <section className="rounded-2xl border bg-white p-4 space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <label className="text-sm block">
            ...
            <p className="mt-1 text-xs text-gray-500">Kolommen: REGNR (reg/regnr/rvg) + pack, optioneel SKU/naam/ZI/AIP.</p>
          </label>

          <label className="text-sm block">
            <div className="font-medium">Staatscourant PDF (groot toegestaan)</div>
            <input type="file" accept=".pdf" onChange={onPickPdf} className="mt-1 block w-full rounded-md border px-3 py-2" />
            <p className="mt-1 text-xs text-gray-500">We verwerken pagina’s in de browser met pdfjs.</p>
          </label>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {progress && (
            <span className="text-sm text-gray-600">
              Pagina {Math.min(progress.done, progress.total)} / {progress.total || "…"}
            </span>
          )}
